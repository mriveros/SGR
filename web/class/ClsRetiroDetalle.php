    ...
    if  (empty($_POST['txtCodigoE'])){$codigoElim=0;}else{$codigoElim=$_POST['txtCodigoE'];}
        ...
        if(isset($_POST['agregar'])){
			...
			alert("la Cantidad ingresada supera el Stock actual del Retiro a realizar. Debe ser menor a '.$cantidadactual.'");
                            window.location="http://aplicaciones.intn.gov.py/SGR/web/retiros/Retiro_Detalles.php";
			 </script>';
                    }  else {   
		...
		alert("Error al insertar el Retiro Detalle. Err(108):'.$query.'");
                ...
                $actualizar= '';
                header("Refresh:0; url=http://aplicaciones.intn.gov.py/SGR/web/retiros/Retiro_Detalles.php");
                }
            }
