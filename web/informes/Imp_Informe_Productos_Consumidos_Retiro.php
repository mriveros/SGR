...
function Header()
{
    ...
    $this->Cell(230,1,'INFORME DE REACTIVOS RETIRADOS CONSUMIDOS',100,100,'C');//Titulo
...
$consulta=pg_exec($conectate,"select pro_nombre,retidet_cantidad,retidet_cantidad_actual,reti_fecha,depar_desc,en_nom || ' ' || en_ape as encargado
from producto pro,retiro ret,retiro_detalle retdet,encargado enc,departamentos_unidad dep, stock st, stock_detalle stdet
where ret.en_cod=enc.en_cod 
and ret.depar_cod=dep.depar_cod
and ret.reti_cod=retdet.reti_cod 
and ret.reti_fecha>='$fechadesde' 
and ret.reti_fecha<='$fechahasta'
and st.stock_cod=stdet.stock_cod
and stdet.pro_cod=pro.pro_cod
and stdet.stockdet_cod=retdet.stockdet_cod 
and ret.depar_cod=$departamento");
...
while($i<$numregs)
{
    ...
    $canactual=pg_result($consulta,$i,'retidet_cantidad_actual');
