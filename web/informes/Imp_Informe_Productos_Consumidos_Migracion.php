...
function Header()
{
    ...
    $this->Cell(230,1,'INFORME DE REACTIVOS MIGRADOS CONSUMIDOS',100,100,'C');//Titulo
...
$consulta=pg_exec($conectate,"select pro_nombre,mig_cantidad,mig_cantidad_actual,mig_fecha,depar_desc,en_nom || ' ' || en_ape as encargado
from producto pro,retiro ret,retiro_detalle retdet,encargado enc,departamentos_unidad dep, stock st, stock_detalle stdet,migracion_producto mig
where mig.en_cod=enc.en_cod 
and mig.depar_cod=dep.depar_cod
and ret.reti_cod=retdet.reti_cod 
and mig.mig_fecha>='$fechadesde' 
and mig.mig_fecha<='$fechahasta'
and st.stock_cod=stdet.stock_cod
and stdet.pro_cod=pro.pro_cod
and mig.retidet_cod=retdet.retidet_cod
and stdet.stockdet_cod=retdet.stockdet_cod 
and mig.depar_cod=$departamento");
...
while($i<$numregs)
{
    ...
    $canactual=pg_result($consulta,$i,'mig_cantidad_actual');
    $encargado=pg_result($consulta,$i,'encargado');
    $pdf->Cell(15,5,$i+1,1,0,'C',$fill);
    $pdf->Cell(65,5,$producto,1,0,'L',$fill);
    $pdf->Cell(25,5,$fechamig,1,0,'C',$fill);
    $pdf->Cell(30,5,$cantidad,1,0,'C',$fill);
    $pdf->Cell(30,5,$canactual,1,0,'C',$fill);
    $pdf->Cell(65,5,$encargado,1,1,'L',$fill);
    $fill=!$fill;
    $i++;
}
